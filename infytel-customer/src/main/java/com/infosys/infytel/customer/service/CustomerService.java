...
import com.infosys.infytel.customer.entity.Plan;
...
public class CustomerService {
	...
	public boolean login(LoginDTO loginDTO) {
		...
		Customer cust = null;
		Optional<Customer> oCustomer = custRepo.findById(loginDTO.getPhoneNo());
	...
	public CustomerDTO getCustomerProfile( Long phoneNo) {
		...
		Customer cust = null;
		Optional<Customer> oCustomer = custRepo.findById(phoneNo);
		if(oCustomer.isPresent()) {
			cust = oCustomer.get();
		}else {
			return null;
